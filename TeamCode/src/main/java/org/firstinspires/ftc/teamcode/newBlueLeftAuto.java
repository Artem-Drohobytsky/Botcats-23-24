...
@Autonomous(name="BLUE_LEFT_AUTO 2", group="cvAutos")
//BLUE TOP STARTING PLACE
public class newBlueLeftAuto extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        Pose2d startPos = new Pose2d(-65, 12, Math.toRadians(180));
        Pose2d depositRight = new Pose2d(-30, 12, Math.toRadians(90));
        Pose2d depositLeft = new Pose2d(-30, 12, Math.toRadians(270));
        Pose2d depositCenter = new Pose2d(-36, 12, Math.toRadians(180));
        Pose2d placeLeft = new Pose2d(-44, 54, Math.toRadians(90));
        Pose2d placeCenter = new Pose2d(-36, 56, Math.toRadians(90));
        Pose2d placeRight = new Pose2d(-28, 54, Math.toRadians(90));
        Pose2d parking = new Pose2d(-60, 50, Math.toRadians(0));
        ...
        while(!opModeIsActive()) {
            if (pipeline.getBlueAnalysis() == camera.SkystonePosition.RIGHT) {
                ...
                beacon = 2;
            } else if (pipeline.getBlueAnalysis() == camera.SkystonePosition.LEFT) {
                telemetry.addData("left", "found item in left box");
                telemetry.update();
                // drive.followTrajectorySequence(purpleLeft);

                beacon = 0;
            } else if (pipeline.getBlueAnalysis() == camera.SkystonePosition.CENTER) {
        ...
        if (opModeIsActive()) {
            ...
            front_left_servo.setPower(0.2);
            front_right_servo.setPower(-0.2);
            back_left_servo.setPower(0.2);
            back_right_servo.setPower(-0.2);
            sleep(2000);
